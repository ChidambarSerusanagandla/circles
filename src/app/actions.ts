...
async function context() {
  if (DEMO_MODE) throw new Error("Use the browser demo controls in demo mode.");
  const user = await getUser();
  requireIdentity(user?.id || null);
  return { db: await supabase(), user: user! };
}
...
export async function reactToMessage(
  ...
  try {
    ...
    const { error } = remove
      ...
      : await db
          .from("message_reactions")
          .insert({ message_id: messageId, profile_id: user.id, reaction });
    if (error && error.code !== "23505")
      return { ok: false, message: "Could not save your reaction." };
    if (!remove && !error) {
...
export async function submitQuestion(
  ...
  try {
    ...
    const { data: question, error } = await db
      .from("questions")
      .insert({ group_id: groupId, author_id: user.id, content })
      .select("id")
      .single();
    if (error)
      return {
        ok: false,
        message:
          "Could not send your question. Join this circle first, then try again.",
      };
    await trackServerEvent("question_submitted", groupId, user.id, question.id);
    revalidatePath("/", "layout");
    return {
      ok: true,
      message: "Question sent. A creator can answer it in the conversation.",
    };
  } catch (error) {
    return failure(error);
  }
  ...
  try {
    ...
    if (error)
      return {
        ok: false,
        message:
          "Could not post. Only this circle’s creators can publish messages.",
      };
    revalidatePath("/", "layout");
...
export async function moderateQuestion(
  ...
  try {
    uuidInput.parse(questionId);
    const { db } = await context();
    const { error } =
      answer === null
        ? await db.rpc("skip_question", { target: questionId })
        : await db.rpc("answer_question", {
            target: questionId,
            answer: answerInput.parse(answer),
          });
    if (error)
      return {
        ok: false,
        message:
          error.code === "23505"
            ...
            : "Could not review this question. Check your creator access.",
      };
    revalidatePath("/", "layout");
    return {
      ok: true,
...
export async function createGroup(input: unknown): Promise<ActionResult> {
  try {
    ...
    const { error } = await db.rpc("create_group", {
      ...
      group_slug: fields.slug,
      group_description: fields.description,
      group_category: fields.category,
    });
    if (error)
      return {
        ok: false,
        message:
          error.code === "23505"
            ? "That circle address is already taken."
            : "Could not create this circle.",
      };
    revalidatePath("/", "layout");
    return {
      ok: true,
      message: "Circle created. You can publish its first message.",
    };
  } catch (error) {
    return failure(error);
  }

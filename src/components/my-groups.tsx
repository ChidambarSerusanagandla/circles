  ...
  if (!user)
    return (
      <div className="page empty">
        <UsersRound size={36} className="empty-icon" />
        <h1>A place for your favorites.</h1>
        <p>Sign in to keep the circles you join in one place.</p>
        <Link className="button primary" href="/profile">
          Sign in{isDemo ? " or try the demo" : ""}
        </Link>
      </div>
    );
  const joined = groups.filter((g) =>
    isDemo ? hasDemoMembership(state, g.id) : memberships.includes(g.id),
  );
  const managed = groups.filter((g) => g.admins.some((a) => a.id === user.id));
  function list(items: Group[], empty: string) {
    return items.length ? (
      ...
      <div className="small-empty">
        ...
        <Link className="text-button" href="/">
          Find a conversation <ArrowUpRight size={15} />
        </Link>
      </div>
    );
  }
  return (
    <div className="page library-page">
      ...
      <section className="library-section">
        ...
        {list(
          joined,
          "You haven’t joined a circle yet. Something good is waiting on Discover.",
        )}
      </section>
      <section className="library-section">
        <h2>

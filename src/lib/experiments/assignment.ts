...
export interface Assignment {visitorId:string;variant:Variant;experimentId:string;active:boolean;attributed?:boolean;}
...
export function variantFor(visitorId:string):Variant {let hash=2166136261;for(const char of `preview-v1:${visitorId}`){hash^=char.charCodeAt(0);hash=Math.imul(hash,16777619);}return (hash>>>0)%2===0?"A":"B";}

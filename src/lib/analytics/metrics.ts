...
function afterPreview(event:AnalyticsEvent,preview:AnalyticsEvent,matchExperiment=true) {const elapsed=Date.parse(event.created_at)-Date.parse(preview.created_at);return event.anonymous_session_id===preview.anonymous_session_id&&event.group_id===preview.group_id&&(!matchExperiment||(event.experiment_variant===preview.experiment_variant&&event.experiment_id===preview.experiment_id))&&elapsed>=0&&elapsed<=SEVEN_DAYS;}
export function funnels(events:AnalyticsEvent[],isDemo:boolean):Funnel[] {
  const eligible=events.filter(e=>e.is_demo===isDemo&&e.experiment_id===PREVIEW_EXPERIMENT_ID);const previews=eligible.filter(e=>e.event_name==="group_preview_seen");
...
export function creatorMetrics(events:AnalyticsEvent[],groupId:string,isDemo:boolean):CreatorMetrics {
 ...
 const opens=eligible.filter(e=>e.event_name==="group_opened"&&previews.some(p=>afterPreview(e,p,false)));
 const joins=eligible.filter(e=>e.event_name==="group_joined"&&previews.some(p=>afterPreview(e,p,false)&&opens.some(o=>afterPreview(o,p,false)&&o.anonymous_session_id===e.anonymous_session_id&&Date.parse(o.created_at)<=Date.parse(e.created_at))));

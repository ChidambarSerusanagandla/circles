...
export async function trackServerEvent(name:EventName,groupId:string|null,userId:string|null,key:string) {
 if(DEMO_MODE)return;
 await bestEffort(async()=>{
  const assignment=await getAssignment();const db=analyticsDatabase();
  const event=makeEvent(assignment,name,groupId,userId,false);

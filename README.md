...
`npm run test:e2e:demo` remains an optional legacy suite requiring an explicitly demo-mode build; it is not a CI or connected-release gate. The preflight rejects a mismatch between the test environment and the running app; never re-enable demo login to satisfy a connected test. Build in the selected mode before testing. Set `PLAYWRIGHT_EXTERNAL_SERVER=true` to reuse an already-running matching server, otherwise Playwright starts the production build.

GitHub Actions runs `npm ci`, lint, TypeScript, unit/PostgreSQL tests, the existing seed dry run, and the production build. It uses demo mode only for credential-free build validation, not as deployment configuration. CI runs neither Playwright suite, contains no hosted Supabase credentials, and does not test against the shared `circles-review` project. Keep `npm run test:e2e:connected` and `npm run test:e2e:cleanup` for manual/pre-release validation. The latest manually verified connected run passed **18 tests, 0 failed, 0 skipped**; subsequent cleanup verification found **0 E2E resources remaining**. See [VERIFICATION.md](VERIFICATION.md) for the evidence and historical checks.
